# adaptive_audio_system.py - EEG + IMU ONLY (NO HR DEPENDENCIES)
...
class BulletproofAudioSystem:
    ...
    UPDATED: Uses only EEG + IMU derived meditation states (no HR data)
    """
    ...
    def __init__(self, sample_rate=44100, buffer_size=4096, target_device="AUDIO_FRENZJ41"):
        ...
        print(f"BULLETPROOF Audio System initializing (EEG + IMU driven)...")
        ...
        # Rule-based controller parameters (EEG + IMU driven)
        ...
        # Modulation rules based on EEG + IMU meditation states
        self.modulation_rules = {
            MeditationState.REST: {
                "volume_db_change": +3,      # Slightly louder for rest state
                "freq_hz_change": +1,        # Higher frequency to encourage focus
                "stereo_balance": +0.3,      # Right ear emphasis
                "freq_jitter_hz": 0.0
            },
            MeditationState.LIGHT: {
                "volume_db_change": 0,       # Baseline volume
                "freq_hz_change": 0,         # Baseline frequency (7Hz)
                "stereo_balance": 0.0,       # Centered
                "freq_jitter_hz": 0.0
            },
            MeditationState.DEEP: {
                "volume_db_change": -2,      # Softer for deep states
                "freq_hz_change": -1,        # Lower frequency (6Hz)
                "stereo_balance": 0.0,       # Centered
                "freq_jitter_hz": 0.5        # Slight natural variation
            }
        }
        ...
        # Smoothing factors for bulletproof transitions
        ...
        print("BULLETPROOF Audio System initialized (EEG + IMU driven)")
        ...
        print(f"   EEG + IMU meditation state input: No HR dependencies")
    ...
    def start_parameter_thread(self):
        ...
        print("Parameter update thread started (EEG + IMU driven)")
    ...
    def parameter_update_worker(self):
        """Background worker that handles parameter updates from EEG + IMU states"""
    ...
    def process_parameter_update(self, update_request):
        """Process parameter update request from EEG + IMU LSTM predictions"""
        ...
        if state != self.current_meditation_state:
            ...
            # Calculate new target parameters based on EEG + IMU state
            ...
            self.logger.info(f"BULLETPROOF parameter update: {state.name} (EEG+IMU confidence: {confidence:.3f})")
    ...
    def generate_bulletproof_binaural_beat(self, n_samples, output_buffer):
        """Generate binaural beats based on EEG + IMU derived meditation state"""
        try:
            ...
            # Add jitter for deep state (detected by EEG + IMU)
    ...
    def audio_callback(self, outdata, frames, time, status):
        ...
        try:
            ...
            # Add bulletproof binaural beats for induction sessions (EEG + IMU driven)
    ...
    def update_meditation_state(self, state, confidence):
        """Queue parameter update from EEG + IMU LSTM predictions (non-blocking)"""
        try:
            ...
            update_request = {
                ...
                'timestamp': time.time(),
                'source': 'eeg_imu_lstm'  # Indicate source is EEG + IMU
            }
    ...
    def start_session(self, session_number):
        """Start bulletproof meditation session (EEG + IMU driven)"""
        if not AUDIO_AVAILABLE:
        ...
        self.logger.info(f" Starting BULLETPROOF Session {session_number} (EEG + IMU driven):")
        ...
        self.logger.info(f"   State input: EEG + IMU only (no HR)")
        ...
        try:
            ...
            self.logger.info("✅ BULLETPROOF audio session started on FRENZ brainband (EEG + IMU driven)")
    ...
    def get_session_progress(self):
        ...
        return {
            ...
            "state_source": "eeg_imu_lstm"  # Indicate state source
        }
...
class MeditationAudioManager:
    """Bulletproof integration wrapper (EEG + IMU driven, no HR)"""
    ...
    def __init__(self, target_device="AUDIO_FRENZJ41"):
        ...
        if self.audio_system.audio_device_id is not None:
            print(f"BULLETPROOF FRENZ brainband audio ready (EEG + IMU driven)!")
            ...
            print(f"   State input: EEG + IMU LSTM predictions (no HR dependencies)")
    ...
    def initialize_session(self, session_number):
        ...
        try:
            ...
            if success:
                self.current_session = session_number
                self.session_start_time = time.time()
                print(f"BULLETPROOF audio session {session_number} started (EEG + IMU driven)!")
    ...
    def update_from_lstm_prediction(self, lstm_prediction):
        """Update with bulletproof parameter handling (from EEG + IMU LSTM)"""
        if lstm_prediction and lstm_prediction.get('ready'):
            pred = lstm_prediction.get('prediction', {})
            state = pred.get('class', 0)
            confidence = pred.get('confidence', 0.5)
            
            # Pass EEG + IMU derived state to audio system
            self.audio_system.update_meditation_state(state, confidence)
    
    def get_status(self):
        """Get bulletproof status"""
        status = self.audio_system.get_session_progress()
        # Add additional metadata about the state source
        if status.get("active"):
            status["state_input_type"] = "eeg_imu_only"
            status["hr_dependencies"] = False
        return status
...
def test_bulletproof_system():
    """Test the bulletproof system (EEG + IMU driven)"""
    if not AUDIO_AVAILABLE:
        print("❌ Cannot test - audio libraries not available")
        return
    
    print("Testing BULLETPROOF Anti-Artifact System (EEG + IMU driven)")
    print("=" * 70)
    ...
    print(f"   State input: EEG + IMU only (no HR)")
    print(f"   TARGET: Zero 'po, po' artifacts")
    
    if audio_manager.initialize_session(4):
        print("BULLETPROOF system started (EEG + IMU driven)!")
        print("Testing rapid state changes that normally cause artifacts...")
        
        # Aggressive test - rapid state changes (simulating EEG + IMU predictions)
        states = [
        ...
        for state, confidence, duration in states:
            print(f"{state.name} state (EEG+IMU derived) → should be artifact-free")
            ...
            for i in range(duration):
                ...
                if status["active"]:
                    print(f"   Freq: {status['binaural_freq']:.3f}Hz, "
                          ...
                          f"Source: {status.get('state_input_type', 'unknown')}")
                time.sleep(1)
        
        print("Stopping BULLETPROOF test")
        audio_manager.stop_current_session()
        print("✅ BULLETPROOF test completed (EEG + IMU driven)!")
