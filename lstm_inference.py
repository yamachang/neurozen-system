# lstm_inference.py - EEG + IMU ONLY (NO PPG/HR)
...
# Import the feature aligner (updated for EEG + IMU only)
...
class LSTMInferenceEngine:    
    def __init__(self, model_dir="./models"):
        ...
        print("Initializing LSTM Inference Engine (EEG + IMU only, no HR)...")
    ...
    def _create_normalization_files(self):
        ...
        try:
            ...
            # Filter out any HR features that might exist in old model
            valid_features = []
            valid_means = []
            valid_scales = []
            
            hr_keywords = ['heart_rate', 'hr_', 'ppg_', 'bpm']
            
            for i, feature_name in enumerate(self.feature_columns):
                feature_lower = feature_name.lower()
                is_hr_feature = any(keyword in feature_lower for keyword in hr_keywords)
                
                if not is_hr_feature:
                    valid_features.append(feature_name)
                    valid_means.append(self.scaler.mean_[i])
                    valid_scales.append(self.scaler.scale_[i])
                else:
                    print(f"[FILTER] Excluding HR feature from normalization: {feature_name}")
            
            print(f"✅ Filtered features: {len(self.feature_columns)} → {len(valid_features)} (removed HR features)")
            
            # Update feature columns to only include valid features
            ...
            for i, feature_name in enumerate(valid_features):
                normalization_stats[feature_name] = {
                    'mean': float(valid_means[i]),
                    'std': float(valid_scales[i]),  # StandardScaler stores std in scale_
                    'initialized': True
                }
            ...
            ml_stats_structure = {
                ...
                'metadata': {
                    ...
                    'feature_types': 'eeg_imu_only_no_hr'
                }
            }
            ...
            print(f"   📊 Created stats for {len(normalization_stats)} features (EEG + IMU only)")
    ...
    def _check_normalization_files_format(self, ml_stats_path, fill_stats_path):
        ...
        try:
            ...
            # Check for HR features in the stats (should not exist)
            hr_keywords = ['heart_rate', 'hr_', 'ppg_', 'bpm']
            feature_stats = ml_data['feature_stats']
            hr_features_found = []
            
            for feature_name in feature_stats.keys():
                feature_lower = feature_name.lower()
                if any(keyword in feature_lower for keyword in hr_keywords):
                    hr_features_found.append(feature_name)
            
            if hr_features_found:
                print(f"🔧 Found HR features in normalization stats, will recreate: {hr_features_found}")
                return True
            ...
            # Files exist and have correct format (and no HR features)
            return False
    ...
    def _verify_normalization_files_format(self):
        ...
        try:
            ...
            if ml_stats_path.exists():
                ...
                # Check for HR features (should not exist)
                hr_keywords = ['heart_rate', 'hr_', 'ppg_', 'bpm']
                hr_features_found = []
                ...
                if hr_features_found:
                    print(f"   ❌ Found HR features in stats (should be removed): {hr_features_found}")
                    return False
                else:
                    print(f"   ✅ No HR features found - clean EEG + IMU only")
    ...
    def _load_model_components(self):
        ...
        try:
            ...
            # Check for HR features and filter them out
            hr_keywords = ['heart_rate', 'hr_', 'ppg_', 'bpm']
            hr_features_found = []
            clean_features = []
            ...
            for feature_name in self.feature_columns:
                ...
                if is_hr_feature:
                    ...
                    clean_features.append(feature_name)
            
            if hr_features_found:
                print(f"   ⚠️  Found {len(hr_features_found)} HR features - filtering out: {hr_features_found}")
                self.feature_columns = clean_features
                print(f"   ✅ Cleaned features: {len(self.feature_columns)} (EEG + IMU only)")
            else:
                print(f"   ✅ No HR features found - already clean")
        ...
        try:
            ...
            if len(input_shape) == 3:  # (batch_size, sequence_length, features)
                ...
                # Verify feature count matches (should be 82 for EEG + IMU)
                if expected_features != len(self.feature_columns):
                    ...
                    print(f"   Expected for EEG + IMU: ~82 features")
        ...
        print(f"   LSTM Inference Engine Ready (EEG + IMU only):")
        print(f"   Model: {os.path.basename(model_path)}")
        print(f"   Features: {len(self.feature_columns)} (no HR)")
        ...
        if results['ml_stats_exists']:
            try:
                ...
                for feature_name in feature_stats.keys():
                    feature_lower = feature_name.lower()
                    if any(keyword in feature_lower for keyword in hr_keywords):
                        results['hr_features_found'].append(feature_name)
...
class AlignedLSTMInferenceEngine(LSTMInferenceEngine):    
    def __init__(self, model_dir="./models"):
        ...
        print("✅ Feature aligner initialized for EEG + IMU alignment (no HR)")
        ...
        if norm_check['ml_stats_exists'] and norm_check['fill_stats_exists']:
            ...
            if norm_check['hr_features_found']:
                print(f"   ⚠️  HR features found in normalization (will be filtered): {norm_check['hr_features_found']}")
            else:
                print(f"   ✅ No HR features found - clean EEG + IMU stats")
    ...
    def predict(self, raw_features):
        try:
            # Align features to match LSTM expectations (EEG + IMU only)
...
def test_aligned_inference_engine():
    """Test the aligned inference engine with dummy data (EEG + IMU only)"""
    print("Testing Enhanced Aligned LSTM Inference Engine (EEG + IMU only)...")
    ...
    try:
        ...
        try:
            ...
            # Test with EEG + IMU features only (no HR)
            simple_features = {
                'lf_abs_theta_power': -35.0,
                'rf_abs_theta_power': -33.0,
                'stillness_score': 0.8,
                'movement_intensity': 150.0
            }
            ...
            print(f"   ✅ No HR features processed")
        ...
        # Test with scenarios that should produce different predictions (EEG + IMU only)
        test_scenarios = [
            {
                'name': 'Low Theta + High Movement (Rest-like)',
                'features': {
                    ...
                    'stillness_score': 0.3,
                    'movement_intensity': 400.0,
                    'movement_count': 8.0
                }
            },
            {
                'name': 'Medium Theta + Moderate Movement (Light meditation)',
                'features': {
            ...
            {
                'name': 'High Theta + Low Movement (Deep meditation)',
                'features': {
        ...
        for scenario in test_scenarios:
            ...
            if result.get('ready') and result.get('prediction'):
                ...
                print(f"   {scenario['name']:40}: {state_label} (Class {pred['class']}, Conf {pred['confidence']:.3f})")
            else:
                print(f"   {scenario['name']:40}: FAILED - {result.get('error', 'Unknown error')}")
        ...
        if len(valid_predictions) == 0:
            ...
            print(f"   Your EEG + IMU model is working correctly!")
            print(f"   Confidences look realistic ({avg_confidence:.1%})")
            return True
        elif unique_predictions == 1:
