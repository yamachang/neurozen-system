...
class FixedNormalizationRealTimeProcessor:   
    ...
    def process_realtime_data(self, eeg_data, ppg_data=None, imu_data=None, sqc_scores=None, 
        ...
        FIXED: Properly handle data orientation and channel selection
        """
        ...
        print(f"  PPG: {ppg_data.shape if ppg_data is not None and hasattr(ppg_data, 'shape') else 'None'}")
        ...
        try:
            ...
            for processed_eeg_epoch, original_quality_flag, epoch_start_s, special_processed in \
                ...
                if final_quality and processed_eeg_epoch is not None:
                    ...
                    ppg_count = sum(1 for k in all_sensor_features.keys() 
                                if any(kw in k for kw in ['heart_rate', 'hr_']))
                    imu_count = sum(1 for k in all_sensor_features.keys() 
                                if any(kw in k for kw in ['stillness', 'movement', 'accel']))
                    
                    print(f"[FEATURES] Extracted: EEG={eeg_count}, PPG={ppg_count}, IMU={imu_count}")
